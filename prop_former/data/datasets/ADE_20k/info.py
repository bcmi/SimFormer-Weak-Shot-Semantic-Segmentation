...
did_list = [d['id'] for d in ADE20K_150_CATEGORIES]
assert len(did_list) == 150, len(did_list)

did_to_cid = {k: i for i, k in enumerate(did_list)}
did_to_name = {d['id']: d['name'] for d in ADE20K_150_CATEGORIES}
did_to_color = {d['id']: d['color'] for d in ADE20K_150_CATEGORIES}

novel_num = round(len(did_list) / 4)
# novel_dids = sorted(np.random.permutation(did_list)[:novel_num].tolist())

# SPLIT 1
split1_novel_dids = [1, 3, 6, 8, 10, 13, 17, 22, 23, 29, 33, 38, 41, 50, 51, 56, 67, 70, 71, 74, 77, 81, 92, 93, 98,
...
split2_novel_dids = [0, 10, 21, 27, 28, 29, 34, 40, 42, 43, 51, 57, 58, 65, 66, 68, 69, 73, 76, 78, 85, 88, 90, 100,
...
split3_novel_dids = [0, 5, 9, 15, 16, 21, 24, 25, 27, 32, 34, 35, 38, 39, 55, 70, 76, 85, 95, 96, 98, 100, 106, 109,
...
split4_novel_dids = [5, 11, 13, 16, 21, 24, 26, 32, 35, 42, 53, 56, 60, 61, 63, 64, 68, 70, 75, 82, 91, 93, 94, 99, 100,
