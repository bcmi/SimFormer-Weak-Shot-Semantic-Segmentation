...
class MFTrainer(DefaultTrainer):
    """
...
def main(args):
    ...
    if args.eval_only:
        model = MFTrainer.build_model(cfg)
        DetectionCheckpointer(model, save_dir=cfg.OUTPUT_DIR).resume_or_load(
            cfg.MODEL.WEIGHTS, resume=args.resume
        )
        res = MFTrainer.test(cfg, model)
        if cfg.TEST.AUG.ENABLED:
            tta_res = MFTrainer.test_with_TTA(cfg, model)
